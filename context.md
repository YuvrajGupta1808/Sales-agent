...
| `Outreach` | 4 lead rows | Four prospect rows are currently populated. |
...
`src/data/trackerSnapshot.json` is generated by:

```bash
npm run sync:xlsx
```

The generated snapshot is the first API-like contract in the repo: workbook rows are normalized into JSON so a future backend can serve the same shape from an endpoint.
...
- Shows four prospect outreach rows and their next actions.
